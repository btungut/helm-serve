...
- Rewrote [`README.md`](README.md) / [`chart/README.md`](chart/README.md): clarified that the project is a Helm library around application-owned images, sharpened the value proposition and the "Architecture and naming" section, and marked Traefik CRD mode as available on `master` but unreleased. The configuration reference now documents previously undocumented parameters (`deployment.name`, `service.name`, `ingress.name`, `ingress.labels`, `cronJob.name`, the `required` flag on `configMaps`/`secrets`, the fixed metrics Service port `9211`) and clarifies that the `PrometheusRule` is rendered together with the `ServiceMonitor`.
- Expanded [`test/README.md`](test/README.md) with a description of how the test harness is wired (one-line `library.*` includes, the two helper prefixes used for `templatePrefix` coverage) and a dependency-refresh step in the contribution workflow.

## [0.2.3-beta2] – 2026-05-09
...
- Updated [`chart/README.md`](chart/README.md) and [`README.md`](README.md) to:
  - Highlight Ingress TLS as the primary delta between v0.2.2 and v0.2.3-beta2.
